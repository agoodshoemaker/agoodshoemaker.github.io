...
The triangle inequality tells us that $a + b - c > 0$. From this we see that $x < 0$. But does this result in a valid triangle? Let us ask the triangle inequality to adjudge the issue:
...
So the triplet $(a + x, b + x, c + x)$ is not a triangle, a contradiction! So it is clear that only one of the two brothers can be right!