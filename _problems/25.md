...
- This equation has infinite solutions. For every value of $x = \alpha$, there is a corresponding value of $y = \beta$ such that the pair $(\alpha, \beta)$ will be a solution to this equation. And this $(\alpha, \beta)$ will correspond to a point that lies on the line $a_2x + b_2 y + c_2 = 0$. Therefore, the entire plane is not the solution, but only points on a line.
...
y &= \cfrac{c_1 a_2 - a_2 c_1}{a_1 b_2 - a_2 b_1}
\end{align*}
$$
...
- Notice that any pair $(x, y) = (\alpha, \beta)$ does satisfy both the equations. But we also know that only those points on the straight line $a_2 x + b_2 y + c_2 = 0$ are solutions to the original system. So where is the discrepancy? If $(\alpha, \beta)$ is a solution to the reduced system, should it not also satisfy the original system? The answer is an emphatic no! 
...
- The one other thing that has been left out is the case where one of the coefficients is zero. 
